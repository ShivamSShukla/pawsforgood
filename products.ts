...
export function getProductLink(product: Product, region: 'US' | 'UK'): string {
  ...
  const usTag = typeof window !== 'undefined' 
    ? process.env.NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG_US 
    : process.env.NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG_US;
  
  const ukTag = typeof window !== 'undefined'
    ? process.env.NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG_UK
    : process.env.NEXT_PUBLIC_AMAZON_ASSOCIATE_TAG_UK;
  
  const tag = region === 'US' 
    ? (usTag || 'pawsforgood-20')
    : (ukTag || 'pawsforgood-21');
  
  const baseUrl = region === 'US' ? product.amazonUrlUS : product.amazonUrlUK;
  
  // Ensure URL has proper format
  if (!baseUrl) {
    console.error(`Missing Amazon URL for product ${product.id} in region ${region}`);
    return region === 'US' ? 'https://www.amazon.com' : 'https://www.amazon.co.uk';
  }
